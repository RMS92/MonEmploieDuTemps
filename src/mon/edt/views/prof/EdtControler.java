...
public class EdtControler implements Initializable{
	...
	private String currentSemaine;
		...
		private void chargerEdt(int semaine) {
			...
			//recherche des seances 
			EdtEnseignantTable eT = new EdtEnseignantTable(Login.getInstance());
			this.e = eT.findAll(this.u.getId(), semaine);
		...
		private void chargerMatiere(Label label, ArrayList<Seance> g, EdtEnseignantTable eT, int i) {
			//NOM MATIERE
			...
			if(nomMatiere.equals("POO Java")) {
			    label.setBackground(new Background(new BackgroundFill(Color.LIGHTGOLDENRODYELLOW, CornerRadii.EMPTY,
					Insets.EMPTY)));
			}
			...
			if(nomMatiere.equals("Probabilités et statistiques")) {
			    label.setBackground(new Background(new BackgroundFill(Color.BURLYWOOD, CornerRadii.EMPTY,
					Insets.EMPTY)));
			}
			...
			if(nomMatiere.equals("Electronique")) {
			    label.setBackground(new Background(new BackgroundFill(Color.CORNFLOWERBLUE, CornerRadii.EMPTY,
					Insets.EMPTY)));
			}
			if(nomMatiere.equals("Electromagnétique")) {
			    label.setBackground(new Background(new BackgroundFill(Color.CADETBLUE, CornerRadii.EMPTY,
					Insets.EMPTY)));
			}
			...
			if(nomMatiere.equals("Amphi d'informations")) {
			    label.setBackground(new Background(new BackgroundFill(Color.INDIANRED, CornerRadii.EMPTY,
					Insets.EMPTY)));
			}
			if(nomMatiere.equals("Droit du travail")) {
			    label.setBackground(new Background(new BackgroundFill(Color.MEDIUMPURPLE, CornerRadii.EMPTY,
					Insets.EMPTY)));
			}
			if(nomMatiere.equals("Anglais") || nomMatiere.equals("Espagnol")) {
			    label.setBackground(new Background(new BackgroundFill(Color.CORAL, CornerRadii.EMPTY,
					Insets.EMPTY)));
			}
			
			///ETAT
			if(etat == 0) {
				label.setBackground(new Background(new BackgroundFill(Color.RED, CornerRadii.EMPTY,
						Insets.EMPTY)));
			}else if(etat == 2) {
				label.setBackground(new Background(new BackgroundFill(Color.GREEN, CornerRadii.EMPTY,
						Insets.EMPTY)));
			}
		...
		private void initLabel(Label label){
			label.setText("");
			label.setBackground(new Background(new BackgroundFill(Color.WHITESMOKE, CornerRadii.EMPTY,
					Insets.EMPTY)));
		}
		...
		private void initTousLabels() {
	        ...
	        //LUNDI
			initLabel(l8);initLabel(l9);initLabel(l10);initLabel(l11);initLabel(l12);initLabel(l13);
			initLabel(l14);initLabel(l15);initLabel(l16);
			//MARDI
			initLabel(m8);initLabel(m9);initLabel(m10);initLabel(m11);initLabel(m12);initLabel(m13);
			initLabel(m14);initLabel(m15);initLabel(m16);
		    //MERCREDI	
			initLabel(me8);initLabel(me9);initLabel(me10);initLabel(me11);initLabel(me12);initLabel(me13);
			initLabel(me14);initLabel(me15);initLabel(me16);
			//jeudi
			initLabel(j8);initLabel(j9);initLabel(j10);initLabel(j11);initLabel(j12);initLabel(j13);
			initLabel(j14);initLabel(j15);initLabel(j16);
			//VENDREDI
			initLabel(v8);initLabel(v9);initLabel(v10);initLabel(v11);initLabel(v12);initLabel(v13);
			initLabel(v14);initLabel(v15);initLabel(v16);
			//SAMEDI
			initLabel(s8);initLabel(s9);initLabel(s10);initLabel(s11);initLabel(s12);
		}
		
		public void chargerCasesEdt(ArrayList<Seance> g, EdtEnseignantTable eT) {
