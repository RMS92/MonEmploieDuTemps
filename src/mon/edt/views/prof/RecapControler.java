...
public class RecapControler implements Initializable{
	...
	//RECAP MATIERE
		...
		public void initialize(URL arg0, ResourceBundle arg1) {
			...
			recapTable.getSelectionModel().selectedItemProperty().addListener(
		...
		private ObservableList<RecapEtudiant> getDataRecap() {
			...
			this.e = rT.findAllDistinctEnseignants(this.u.getId());
			...
			for(int i = 0; i < this.e.size(); i++) {			
				...
				//fction nbCours
				this.nbrCours = rT.findNbHeureEnseignant(this.u.getId(), this.c.getId());
				this.totalH = this.totalH + this.nbrCours;
				
				this.duree = String.valueOf(this.nbrCours) + " h";
				
				this.list.add(new RecapEtudiant(this.c.getNom(), this.premierCours, this.dernierCours,
						this.duree, this.nbrCours));
			}
		...
		private ObservableList<RecapShowEtudiant> getDataSeance() {
			...
			enew = sT.findAllEnseignant(unew.getId(), coursSelection.getText());
